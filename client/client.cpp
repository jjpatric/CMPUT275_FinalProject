...
//global variable to keep track of index in waypoint array
...
bool waitingForEdges = true;

void saveNumWaypoints(){
  shared.num_waypoints = atol(&buffer[2]); // converts number of waypoints sting and saves
  if(shared.num_waypoints == 0){ status_message("NO PATH"); }
  Serial.flush();
  Serial.print("A\n");
  Serial.flush();
...
void addWaypoint(){
  shared.waypoints[waypointIndex].lat = atol(&buffer[2]);
  shared.waypoints[waypointIndex].lon = atol(&buffer[10]);
  waypointIndex++;
  if(waypointIndex > 500){
    status_message("NO PATH");
    waypointIndex = 0; // start overwriting waypoints to stop using too much memory
  }
  Serial.flush();
  Serial.print("A\n");
  Serial.flush();
...
void endPath(){
  waypointIndex = 0;
  waitingForEdges = false;
}

void process_line() {
  
  if(buffer[0] == 65){ successHS = true; } //only checks first letter of handshake message
  else if(buffer[0] == 78){ saveNumWaypoints(); } // if first character of message is 'N'
  else if(buffer[0] == 87){ addWaypoint(); } // if first character of message is 'W'
  else if(buffer[0] == 69){ endPath(); } // if first character of message is 'E'
...
void setup() {
  ...
  status_message("WAIT FOR SERVER");
...
void drawRoute(){
    ...
    {
        /*if(shared.waypoints[i].lon > shared.map_coords.x - 50 &&
        shared.waypoints[i].lon < shared.map_coords.x + displayconsts::tft_width + 50){

            if(shared.waypoints[i].lat > shared.map_coords.y - 50 &&
                shared.waypoints[i].lat < shared.map_coords.y + displayconsts::tft_height + 50){
*/
                tft.drawLine(longitude_to_x(shared.map_number, shared.waypoints[i].lon) - 
                    shared.map_coords.x,

                 latitude_to_y(shared.map_number, shared.waypoints[i].lat) - 
                 shared.map_coords.y,
...
void waitForEdges(){
  while(waitingForEdges){

    read_line();
  }
  waitingForEdges = true;
}
...
int main() {
  ...
  while (true) {
    ...
    // the cursor and map view, the redrawing occurs at the end of this read_line
    process_input();


    // if a zoom button was pushed, update the map and cursor view values
    // for that button push (still need to redraw at the end of this read_line)
